class WineForm
  ...
  :selling_price, :stock, :onlist, :state, :country_id, :shop_id, :wine_id, :grape_ids, :wholesaler_id, :winary_id, :wine_type
  ...
  def save
    ...
    wine = Wine.find_or_create_by(wine_name: wine_name, wine_name_kana: wine_name_kana, vintage: vintage, comment: comment, memo: memo, purchase_price: purchase_price, wine_type: wine_type,
  ...
  def update
    ActiveRecord::Base.transaction do
      ...
      # grape = Grape.find_by(grape_name: grape_name )
  ...
  def set_attributes(wine)
    ...
    self.grape_name = wine.grapes.pluck(:grape_name).split(",")
    self.wine_id = wine.id
    self.grape_ids = wine.grapes.pluck(:id)
